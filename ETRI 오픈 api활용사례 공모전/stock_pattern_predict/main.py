...
def pattern():
    ...
    real_profit = pt.RealProfit(p)
    real_profit.calculate_profit()
    ...
    if len(preds) > 0:
        ...
        max_ = preds.max() * 100
        size_ = len(preds)
        real_ = real_profit.calculate_profit()
        print(avg_, min_, max_, size_)
        return render_template('result.html', code=code, startdate=startdate, enddate=enddate, avg=round(avg_, 2), min=round(min_, 2), max=round(max_, 2), size=size_, real=real_)
    else:
